...
### platform/main.roc
- [ ] 定义 Roc 平台接口
- [ ] 导出 main_for_host 函数给 Zig 宿主调用
- [ ] 支持基本的字符串返回

### src/host.zig
- [ ] 实现 Roc 分配器接口 (roc_alloc, roc_dealloc, roc_realloc)
- [ ] 实现 roc_panic 处理
- [ ] 实现 Solana 入口点 (entrypoint)
- [ ] 使用 solana-program-sdk-zig 的 allocator 和 log 模块
- [ ] 调用 sol_log 输出日志

### examples/hello-world/app.roc
- [ ] 使用 Solana 平台
- [ ] 返回 "Hello from Roc on Solana!" 字符串

### build.zig
- [ ] 配置 BPF 目标编译
- [ ] 使用 solana-program-sdk-zig 的构建模式
- [ ] 集成 sbpf-linker 链接步骤
- [ ] 生成可部署的 .so 文件

### 测试和部署
- [ ] Zig 构建成功 (zig build test)
- [ ] 生成有效的 BPF 字节码 (zig build solana)
- [ ] 部署到本地测试网成功
- [ ] 调用程序并验证日志输出 ✅ "Hello Roc on Solana!"

### 文档
- [ ] 更新 README.md 包含使用说明
- [ ] 创建部署脚本 (scripts/deploy.sh)
- [ ] 创建调用脚本 (scripts/invoke.sh)
